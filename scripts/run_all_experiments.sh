...
#    JOB=A3 VANILLA_CONFIG=configs/legacy/rat_vanilla_benchmark.yaml sbatch scripts/run_all_experiments.sh
#
#  ── Phase 2 — ID/OOD Analysis ─────────────────────────────────
#  Each B-job delegates entirely to its config in configs/legacy/id_ood_b*.yaml.
...
if [ "$JOB" = "A1" ] || [ "$JOB" = "A2" ]; then
    CFG=${VANILLA_CONFIG:-configs/legacy/nhp_vanilla_benchmark.yaml}
    echo "[$(date)] $JOB — NHP vanilla optimization config=$CFG ${VANILLA_OVERRIDES:+overrides=$VANILLA_OVERRIDES}"
    srun python -m pfns4neurostim.legacy_code.vanilla_benchmark --config "$CFG" --mode optimization --save $VANILLA_OVERRIDES

elif [ "$JOB" = "A3" ]; then
    CFG=${VANILLA_CONFIG:-configs/legacy/rat_vanilla_benchmark.yaml}
    echo "[$(date)] A3 — Rat vanilla benchmark config=$CFG ${VANILLA_OVERRIDES:+overrides=$VANILLA_OVERRIDES}"
    srun python -m pfns4neurostim.legacy_code.vanilla_benchmark --config "$CFG" --save $VANILLA_OVERRIDES

elif [ "$JOB" = "A4" ]; then
    CFG=${VANILLA_CONFIG:-configs/legacy/nhp_vanilla_benchmark.yaml}
    echo "[$(date)] A4 — NHP budget sweep config=$CFG ${VANILLA_OVERRIDES:+overrides=$VANILLA_OVERRIDES}"
    srun python -m pfns4neurostim.legacy_code.vanilla_benchmark --config "$CFG" --mode optimization_budget --save $VANILLA_OVERRIDES

# ── Phase 2 — ID/OOD Analysis ─────────────────────────────────────────────────

elif [ "$JOB" = "B1" ]; then
    CFG=${B_CONFIG:-configs/legacy/id_ood_b1.yaml}
    echo "[$(date)] B1 — Shannon Entropy datasets=$DATASETS config=$CFG"
    srun python -m pfns4neurostim.legacy_code.id_ood_analysis --config "$CFG" --datasets $DATASETS

elif [ "$JOB" = "B2" ]; then
    CFG=${B_CONFIG:-configs/legacy/id_ood_b2.yaml}
    echo "[$(date)] B2+B5 — MMD + Wasserstein-2 datasets=$DATASETS config=$CFG"
    srun python -m pfns4neurostim.legacy_code.id_ood_analysis --config "$CFG" --datasets $DATASETS

elif [ "$JOB" = "B3" ]; then
    CFG=${B_CONFIG:-configs/legacy/id_ood_b3.yaml}
    echo "[$(date)] B3 — Mahalanobis Distance datasets=$DATASETS config=$CFG"
    srun python -m pfns4neurostim.legacy_code.id_ood_analysis --config "$CFG" --datasets $DATASETS

elif [ "$JOB" = "B4" ]; then
    CFG=${B_CONFIG:-configs/legacy/id_ood_b4.yaml}
    echo "[$(date)] B4 — Layer-wise CKA (layers 4,13,17) datasets=$DATASETS config=$CFG"
    srun python -m pfns4neurostim.legacy_code.id_ood_analysis --config "$CFG" --datasets $DATASETS

elif [ "$JOB" = "B4dense" ]; then
    CFG=${B_CONFIG:-configs/legacy/id_ood_b4dense.yaml}
    echo "[$(date)] B4dense — Dense Layer-wise CKA (10-layer sweep) datasets=$DATASETS config=$CFG"
    srun python -m pfns4neurostim.legacy_code.id_ood_analysis --config "$CFG" --datasets $DATASETS

elif [ "$JOB" = "B6" ]; then
    CFG=${B_CONFIG:-configs/legacy/id_ood_b6.yaml}
    echo "[$(date)] B6 — RSA datasets=$DATASETS config=$CFG"
    srun python -m pfns4neurostim.legacy_code.id_ood_analysis --config "$CFG" --datasets $DATASETS

elif [ "$JOB" = "B7" ]; then
    CFG=${B_CONFIG:-configs/legacy/id_ood_b7.yaml}
    echo "[$(date)] B7 — Procrustes BO-Trajectory datasets=$DATASETS config=$CFG"
    srun python -m pfns4neurostim.legacy_code.id_ood_analysis --config "$CFG" --datasets $DATASETS

elif [ "$JOB" = "Bfull" ]; then
    CFG=${B_CONFIG:-configs/legacy/id_ood_bfull.yaml}
