"""Finetuned and gradient-monitored TabPFN regressors (legacy; task #1 Step 10).

Moved out of the main API: finetuning was cut from the research design on
2026-09-16 (the old Hyp C), so these classes are kept importable for reproducing
earlier results but are not part of the package's supported surface. Tests that
cover them are marked ``legacy``.
"""
...
class LoRAFinetunedRegressor(GradientMonitoredRegressor):
    ...
    def fit(self, X, y, **kwargs):
        ...
        from pfns4neurostim.legacy_code.finetuning.lora import get_lora_state_dict
        result = super().fit(X, y, **kwargs)
    ...
    def _log_epoch_evaluation(self, epoch, eval_result, mean_train_loss):
        ...
        """
        if epoch >= 0:
            from pfns4neurostim.legacy_code.finetuning.lora import get_lora_state_dict, LoRALinear
            model = self.finetuned_estimator_.model_
