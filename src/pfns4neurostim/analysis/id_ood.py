...
def run_id_ood_analysis(dataset_types=None, analyses=None,
    ...
    Args:
        ...
        analyses: list of analysis names (default: ['entropy', 'mahalanobis']).
            Additional: 'gradient_norm', 'rsa', 'procrustes'. 'mmd', 'wasserstein' and
            'cka' were removed (tasks #5/#6) and raise.
        prior_source: 'gp' | 'tabpfn_prior' | 'both'
        device: 'cpu' or 'cuda'
        ...
        cka_layers: transformer layer indices for CKA analysis. None uses
            ID_OOD_LAYERS default (the CKA analysis itself was removed, task #5).
    ...
    removed = sorted(set(analyses) & {'mmd', 'wasserstein', 'cka'})
    if removed:
        raise ValueError(
            f"ID/OOD analyses {removed} were removed (audit F4/F5/F9/F10: random row pairing, "
            "pooled-point marginals, unstandardized noise bank, per-pair bandwidth). Use "
            "`python -m pfns4neurostim mechanism` with analysis: placement (MMD / sliced W2, "
            "task #6) or analysis: cka (task #5)."
        )
